...
size_t fs_read(int fd, void *buf, size_t len)
{
    ...
    if (len == 0) return 0;
        ...
        ide_read(buf, files[fd].offset + file_table[fd - 3].disk_offset, len);
        files[fd].offset += len;
        return len;
    } 
    else {
...
{
	//("Please implement fs_lseek at fs.c");
	//return -1;
	assert(fd > 2);
    assert(fd < NR_FILES + 3);
    switch(whence) {
        ...
        case SEEK_CUR: {
            uint32_t pos = (long)files[fd].offset + offset;
            if (pos >= 0 && pos <= file_table[fd - 3].size) {
                files[fd].offset = pos;
                return files[fd].offset;            
            }
            break;       
        ...
        case SEEK_END: {
            uint32_t pos = (long)file_table[fd - 3].size + offset;
            if (pos >= 0 && pos <= file_table[fd - 3].size) {
                files[fd].offset = pos;
                return files[fd].offset;
            }
