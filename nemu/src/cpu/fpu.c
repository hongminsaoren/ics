...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs)
{
	...
	if ((sig_grs >> (23 + 3)) > 1 || exp < 0)
	{
			   ...
			   )
		{
			...
			if ((sig_grs & 0x1) == 1) sig_grs = (sig_grs >> 1) | 0x1;
			else if (((sig_grs>>1) & 0x1) == 1) sig_grs = (sig_grs >> 1) | 0x1;
			else sig_grs = sig_grs >> 1;
			exp++;
		}
...
uint32_t internal_float_add(uint32_t b, uint32_t a)
{
	...
	if (fa.exponent > fb.exponent) //fa.val的阶数小
	{
		fa.val = b;
		fb.val = a;
	}
